...
In der folgenden Grafik werden die einzelnen Schichten, angefangen von der VueApp, über den ServiceManager und den ListenService von Speech-Vue, die Listen-Komponente in Speech-Framework, das Plugin für ASR (Speech-to-Text), bis hinunter zur Standardschnittstelle des Browsers für SpeechRecognition, dargestellt.
...
Um den ListenService importieren zu können, muss in der jeweiligen Komponente folgende Zeile eingefügt werden:
	...
	import { SPEECH_LISTEN_SERVICE, ServiceManager, ListenService } from 'speech-vue'
...
Beispiel-Komponente für die Integration von Sprache:
 	...
 	<template>
		<div>
			<button @click="listenGerman()">
				Spracheingabe starten
			</button>
			<button @click="listenStop()">
	...
	import { SPEECH_LISTEN_SERVICE, ServiceManager, ListenService, LISTEN_DE_LANGUAGE, LISTEN_EN_ALANGUAGE } from 'speech-vue';

	@Component
	export class ListenComponent extends Vue {
		...
		// eigene Funktionen fuer die Spracheingabe in einer eigenen Vue-Komponente
